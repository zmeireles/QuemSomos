  ...
  return (
    <div className="flex items-center justify-center gap-4">
      {Array.from({ length: totalDots }).map((_, index) => (
        <button
          key={index}
          className={`mx-2 rounded-full focus:outline-none transition-all duration-300 ease-in-out relative ${
            index === activeDot ? 'cursor-default' : 'cursor-pointer'
          }`}
          style={{
        ...
        >
          <span
            ...
            style={{
              ...
              transform: `scale(${
                index === activeDot ? activeDotSize / inactiveDotSize : 1
              })`,
            }}
          ></span>
          {index !== activeDot && (
